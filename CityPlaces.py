...
def show_map(city):
    ...
    try:
        ...
        more = st.button('hotels', key='more')
        if more:
            switch_page('hotels')
        more1=st.button('Famous Food',key='more1')
        if more1:
            st.session_state['selected_city'] = city
            switch_page('Famous Food')
...
def main():
    st.title('Explore Cities and Famous Food')
    city_list = locs['City'].unique().tolist()
